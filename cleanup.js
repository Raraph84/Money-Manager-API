...
tasks.addTask(async (resolve) => {
    ...
    for (const flowsLink of flowsLinks) {
        ...
        if (!flow)
            console.log(`Flow of flows link ${flowsLink.id} does not exist.`);

        if (flowsLink.inflow && !inflow)
            console.log(`Inflow of flows link ${flowsLink.id} does not exist.`);

        if (flowsLink.outflow && !outflow)
            console.log(`Outflow of flows link ${flowsLink.id} does not exist.`);
    }
