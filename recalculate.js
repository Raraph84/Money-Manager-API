...
tasks.addTask(
    async (resolve) => {
        ...
        for (const person of people) {
            ...
            for (const inflow of inflows.filter((inflow) => inflow.person === person.id)) balance += inflow.amount;

            for (const outflow of outflows.filter((outflow) => outflow.person === person.id)) balance -= outflow.amount;
            ...
            if (person.balance === balance) {
                ...
                console.log(
                    `Person ${person.name} (${person.id}) balance is incorrect. (${person.balance} € should be ${balance} €)`
                );
        ...
        for (const account of accounts) {
            ...
            for (const flow of flows.filter((flow) => flow.fromAccount === account.id)) balance -= flow.amount;

            for (const flow of flows.filter((flow) => flow.toAccount === account.id)) balance += flow.amount;
            ...
            if (account.balance === balance) {
                ...
                console.log(
                    `Account ${account.name} (${account.id}) balance is incorrect. (${account.balance} € should be ${balance} €)`
                );
