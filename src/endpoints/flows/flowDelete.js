...
module.exports.run = async (request, database) => {
    ...
    try {
        ...
        if (flow.fromAccount) await database.query("UPDATE accounts SET balance=ROUND(balance+?, 2) WHERE account_id=?", [flow.amount, flow.fromAccount]);
        if (flow.toAccount) await database.query("UPDATE accounts SET balance=ROUND(balance-?, 2) WHERE account_id=?", [flow.amount, flow.toAccount]);
    } catch (error) {
